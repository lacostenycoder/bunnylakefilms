...
(function() {
  $(document).on('ready page:load', function() {
    var cells, desired_width, post_data, search_submit, table_width;
    $('form[method=get]:not([data-remote])').on('submit', function(event) {
      event.preventDefault();
      return Turbolinks.visit(this.action + '?' + $(this).serialize());
    });
    $.standardCookie = function(name, value) {
      return $.cookie(name, value, {
        path: '/',
        expires: 730
      });
    };
    $.standardDataAttributeCookieBinding = function(data_attribute, cookie_name) {
      return $("a[" + data_attribute + "]").click(function(event) {
        event.preventDefault();
        $.standardCookie(cookie_name, $(this).attr(data_attribute));
        Turbolinks.visit(window.location.href);
      });
    };
    $('select.select_small_field').chosen({
      disable_search_threshold: 160,
      width: '150px'
    });
    ...
    search_submit = function(event) {
      event.preventDefault();
      return $('#index_search_form').submit();
    };
    $('.search-icon').click(function(event) {
      return search_submit(event);
    });
    $('.clear-search-field').click(function(event) {
      $('#current_search').val('');
    ...
    if ($('.sortable').length > 0) {
      ...
      $('.sortable').sortable({
        ...
        update: function(e, ui) {
          var item_id, row_order;
          item_id = ui.item.data('item-id');
          row_order = ui.item.index();
          post_data[$('.sortable').data('sortable-singular-name')] = {
            row_order_position: row_order
          };
          return $.ajax({
            type: 'POST',
    ...
    $('#portfolio_check_all').change(function(event) {
      return $('.portfolio_ids').prop('checked', this.checked);
    });
    $("a[data-remote]").on("ajax:error", function(e, xhr, status, error) {
      return alert("Something went wrong");
    });
    return $(".keyword_assignment_delete").on("ajax:success", function(e, data, status, xhr) {
  ...
  $(document).ready(function() {
    return setInterval(function() {
      if ($('#work_category_id_chosen a span').text() !== 'Docs + Features') {
        return $('#work_work_status_id_chosen .chosen-drop').hide();
      } else {
        return $('#work_work_status_id_chosen .chosen-drop').show();
      }
    }, 500);
  });
